...
type Props = {
  ...
  varint?:"create"|"update"
}

const Editor = ({varint="create",
  onSubmit,
  onCancel,
  defaultValue=[],disabled=false,innerRef,
  placeholder="Write something...",

}: Props) => {
  const [text,setText]=useState("");
  const [image,setImage]=useState<File|null>(null)
  const [isToolbarVisble,setIToolbarVisible]=useState(true);
  ...
  const isEmpty=!image && text.replace(/<(.|\n)*?>/g,"").trim().length===0;
    const toggleToolbar=()=>{
      setIToolbarVisible((current)=>!current);
  ...
  return (
    <div className='flex flex-col'>
      ...
      <div className={cn('flex flex-col border border-slate-200 rounded-md overflow-hidden focus-within:border-slate-300 focus-within:shadow-sm transition bg-white',
        ...
        <div className="flex px-2 bp-2 z-[5]">
          <Hint label={isToolbarVisble?'Hide formating':"Show formating"}>
      ...
      <Hint label='Image'>
      <Button disabled={disabled} variant={"ghost"} size={"iconSm"} onClick={()=>imageElementRef.current?.click()}>
      ...
      disabled={disabled||isEmpty}  size={"iconSm"}
